...
if ( ! function_exists( 'kct_posted_on' ) ) :
	...
	function kct_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'kct' ),
...
if ( ! function_exists( 'kct_posted_by' ) ) :
	...
	function kct_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'kct' ),
	...
	function kct_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'kct' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'kct' ),
					array(
						'span' => array(
							'class' => array(),
						),
