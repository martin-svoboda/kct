...
$details_position = get_post_meta( get_the_ID(), 'details_position', true ) ?: 'sidebar';
$image_url        = '';
...
<article id="post-<?php echo $event_id; ?>" class="event-post">
	...
	<div class="event-content-wrap">
		<div class="entry-content">
			...
			// Vkládací widget Turinky. Vrací prázdný řetězec, když akci Turinka
			// nezná nebo neodpovídá — stránka akce na něm nesmí stát.
			echo kct_container()->get( \Kct\Turinka\Embed::class )->snippet( $event );
		...
		<?php if ( 'hide' !== $details_position ) : ?>
			<div class="event-<?= $details_position ?>">
				...
				if ( isset( $event['contact'] ) && ! empty( $event['contact'] ) ) : ?>
					...
					?>
				<?php endif;
				if ( isset( $event['proposal'] ) && ( ! empty( $event['proposal']['url'] ) || isset( $event['proposal'][0]['url'] ) ) ) : ?>
					<h3>Propozice</h3>
					<p>
						<?php $icon = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 20 20">
						<path fill="currentColor"
						...
						if ( ! empty( $event['proposal']['url'] ) ) {
							printf( '%s <a href="%s" target="_blank">%s</a>', $icon, $event['proposal']['url'], $event['proposal']['name'] );
						} else {
							foreach ( $event['proposal'] as $item ) {
								printf( '<p>%s <a href="%s" target="_blank">%s</a></p>', $icon, $item['url'], $item['name'] );
							}
						}
						?>
					</p>
