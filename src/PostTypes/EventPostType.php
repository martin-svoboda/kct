...
class EventPostType extends AbstractCustomPostType {
	...
	public function setup() {
		...
		$items = array_merge( $items, $this->turinka_items() );
	...
	/**
	 * Volba, jestli akce půjde do Turinky.
	 *
	 * Bez propojeného webu se nezobrazuje — pole, které nemá žádný účinek, jen
	 * mate. Popisek říká nahlas, že u akce z centrální databáze se rozdíl mezi
	 * oběma zapnutými stavy neprojeví: viditelnost tam patří centrální databázi
	 * a zápisové API ji u těchhle akcí nepřijímá.
	 */
	private function turinka_items(): array {
		if ( ! $this->turinka->has_token() ) {
			return array();
		}
		...
		return array(
			array(
				'type'    => 'select',
				'id'      => EventState::META_STATE,
				'title'   => __( 'Akce v Turince', 'kct' ),
				'desc'    => __( 'Odešle se do Turinky text akce a odkaz na tuhle stránku, takže SEO zůstane webu. „Jen na odkaz“ znamená, že akce v Turince je a dá se na ni odkázat, ale nechodí do jejích výpisů — u akcí z centrální databáze KČT se ale tenhle rozdíl neprojeví, tam o viditelnosti rozhoduje centrální databáze.', 'kct' ),
				'options' => $options,
			),
		);
	}
