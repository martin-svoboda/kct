...
//		} );
	 ...
	 *
	 * Exporty ve složce, jejíž adresu drží nastavení (`db_export_url`):
	 * akceexport1.php  jen změny akcí v poslední době
	 * akceexport1x.php všechny dostupné akce
	 * akceexport2.php  seznam oblastí
	 * akceexport3.php  seznam odborů
	 * akceexport4.php  typy akcí
	 *
	...
	public function import_db_events( $just_updated = false ) {
		...
		$url = $this->settings->export_url( ( $just_updated ? 'akceexport1' : 'akceexport1x' ) . '.php' );

		if ( ! $url ) {
			$this->warn_missing_export_url();
	 ...
	 *
	 * This method retrieves the `akceexport4.php` XML file from the export URL kept in settings,
	...
	public function import_event_types() {
		$url = $this->settings->export_url( 'akceexport4.php' );
...
// Akce seřadíme podle data
	...
	/**
	 * Upozorní na chybějící adresu exportu, když import běží z příkazové řádky.
	 *
	 * Bez adresy se import jen tiše vrátí — u cronu je to správně, ale kdo
	 * spustí `wp kct import_events` ručně, by jinak koukal na prázdný výstup
	 * a nevěděl, že mu chybí nastavení.
	 *
	 * @return void
	 */
	private function warn_missing_export_url(): void {
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			\WP_CLI::warning( __( 'Adresa exportu z Databáze akcí KČT není vyplněná v Nastavení → KČT. Import se nespustil.', 'kct' ) );
		}
	}
